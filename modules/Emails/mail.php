...
function send_mail($module,$to_email,$from_name,$from_email,$subject,$contents,$cc='',$bcc='',$attachment='',$emailid='',$logo='')
{
	...
	$from_email_field = $adb->query_result($result,0,'from_email_field');
        ...
        if ($_REQUEST['module'] != 'MailManager'){
        if(isUserInitiated()) {
	...
	setCCAddress($mail,'bcc',$bcc);
        ...
        /*
         *  @VGS modified to allow the reply to be the same address as the login user.
...
function setMailerProperties($mail,$subject,$contents,$from_email,$from_name,$to_email,$attachment='',$emailid='',$module='',$logo='')
{
	...
	//Handle the from name and email for HelpDesk
        ...
        //@VGS Global to allow sending emails form mandrill
	
	$rs = $adb->pquery("select first_name,last_name, email1 from vtiger_users where user_name=?", array($from_name));
	$num_rows = $adb->num_rows($rs);
	if($num_rows > 0){
            $from_name = getFullNameFromQResult($rs, 0, 'Users');
            $from_email=$adb->query_result($rs, 0, 'email1');
...
function isUserInitiated() {
	return (($_REQUEST['module'] == 'Emails' || $_REQUEST['module'] == 'Webmails' || $_REQUEST['module'] == 'MailManager') &&
                        ($_REQUEST['action'] == 'mailsend' || $_REQUEST['action'] == 'webmailsend' || $_REQUEST['action'] == 'Save' || $_REQUEST['action'] =='MailManagerAjax'));
}
