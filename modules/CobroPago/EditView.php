...
if (!empty($_REQUEST['related_id']) and ($_REQUEST['return_module']=='Invoice' or $_REQUEST['return_module']=='Quotes' or $_REQUEST['return_module']=='SalesOrder')) {
	...
	$_REQUEST['parent_id']=$adb->getOne('select accountid from vtiger_'.$lermod.' where '.$lermods.'id='.$_REQUEST['related_id']);
}
if (!empty($_REQUEST['related_id']) and $_REQUEST['return_module']=='PurchaseOrder') {
	$_REQUEST['parent_id']=$adb->getOne('select vendorid from vtiger_purchaseorder where purchaseorderid='.$_REQUEST['related_id']);
}
if($record) {
	$focus->id = $record;
	$focus->mode = 'edit';
