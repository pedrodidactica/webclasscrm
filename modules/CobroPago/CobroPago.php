...
class CobroPago extends CRMEntity {
	...
	public static function calculateRelatedTotals($pid) {
	  ...
	  if ($parent_module=='Accounts' and self::account_control_installed()) {
		$sumamountcredit =$adb->getOne("select sum(amount) as suma from vtiger_cobropago inner join vtiger_crmentity on crmid=cobropagoid where deleted=0 and credit=1 and parent_id=$pid");
		$sumamountdebit =$adb->getOne("select sum(amount) as suma from vtiger_cobropago inner join vtiger_crmentity on crmid=cobropagoid where deleted=0 and credit=0 and parent_id=$pid");
		$sumpendingcredit=$adb->getOne("select sum(amount) as suma from vtiger_cobropago inner join vtiger_crmentity on crmid=cobropagoid where deleted=0 and credit=1 and parent_id=$pid and paid='0'");
		$sumpendingdebit=$adb->getOne("select sum(amount) as suma from vtiger_cobropago inner join vtiger_crmentity on crmid=cobropagoid where deleted=0 and credit=0 and parent_id=$pid and paid='0'");
	...
	function trash($module,$record) {
		global $adb;
		$relatedId = $adb->getOne("select related_id from vtiger_cobropago where cobropagoid=$record");
		$pid = $adb->getOne("select parent_id from vtiger_cobropago where cobropagoid=$record");
	...
	function unlinkRelationship($id, $return_module, $return_id) {
		global $adb;
		$relatedId = $adb->getOne("select related_id from vtiger_cobropago where cobropagoid=$id");
		$pid = $adb->getOne("select parent_id from vtiger_cobropago where cobropagoid=$id");
	...
	function vtlib_handler($modulename, $event_type) {
		if($event_type == 'module.postinstall') {
			...
			$modInvoice->addLink('DETAILVIEWBASIC','Add Payment','index.php?module=CobroPago&action=EditView&related_id=$RECORD$&return_module=Invoice&return_id=$RECORD$&return_action=DetailView');
			$modSO->setRelatedList($modCyP, 'CobroPago', Array('ADD'),'get_dependents_list');
			$modSO->addLink('DETAILVIEWBASIC','Add Payment','index.php?module=CobroPago&action=EditView&related_id=$RECORD$&return_module=SalesOrder&return_id=$RECORD$&return_action=DetailView');
			$modPO->setRelatedList($modCyP, 'CobroPago', Array('ADD'),'get_dependents_list');
			$modPO->addLink('DETAILVIEWBASIC','Add Payment','index.php?module=CobroPago&action=EditView&related_id=$RECORD$&return_module=PurchaseOrder&return_id=$RECORD$&return_action=DetailView');
			$modQt->setRelatedList($modCyP, 'CobroPago', Array('ADD'),'get_dependents_list');
			$modQt->addLink('DETAILVIEWBASIC','Add Payment','index.php?module=CobroPago&action=EditView&related_id=$RECORD$&return_module=Quotes&return_id=$RECORD$&return_action=DetailView');
	...
	**/
	function permissiontoedit()
	{
		...
		require('user_privileges/user_privileges_'.$current_user->id.'.php');
		$Block_paid = $adb->getone("select block_paid from vtiger_cobropagoconfig");
