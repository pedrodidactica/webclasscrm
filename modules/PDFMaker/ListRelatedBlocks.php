...
if ($PDFMaker->CheckPermissions("DELETE")) {
    $deleting = true;
...
$sql = "SELECT * FROM vtiger_pdfmaker_relblocks WHERE secmodule IN(" . generateQuestionMarks($Related_Modules) . ") ORDER BY relblockid";
...
while ($row = $adb->fetchByAssoc($result)) {
    ...
    if ($editing)
        $Edits[] = "<a href='index.php?module=PDFMaker&action=PDFMakerAjax&file=EditRelatedBlock&record=" . $row["relblockid"] . "'>" . $app_strings["LBL_EDIT"] . "</a>";

    if ($deleting)
        $Edits[] = "<a href='javascript: deleteRelBlock(" . $row["relblockid"] . ");'>" . $app_strings["LBL_DELETE"] . "</a>";
...
$smarty->display(vtlib_getModuleTemplate($currentModule, 'ListRelatedBlocks.tpl'));
