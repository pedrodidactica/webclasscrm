...
function its4you_if($param1,$comparator,$param2,$whatToReturn1,$whatToReturn2=''){
	...
	if(in_array($comparator, array('==','!=','>=','<=','>','<')))
		return html_entity_decode(eval("if('$param1' $comparator '$param2'){return '$whatToReturn1';} else {return '$whatToReturn2';}"),ENT_COMPAT,'utf-8');
	else
...
{
  global $PDFMaker_template_id, $adb;
...
function its4you_multiplication()
{
	...
	if(!empty($input_args)){
		foreach($input_args as $key=>$sum){
			$sum = strip_tags($sum);
			$sum = its4you_formatNumberFromPDF($sum);
