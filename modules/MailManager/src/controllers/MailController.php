...
class MailManager_MailController extends MailManager_Controller {
					...
					/*if(is_array($attachments)) {
						foreach($attachments as $attachment){
							...
							if(is_file($fileNameWithPath)) {
								$attach->AddAttachment($fileNameWithPath, $attachment['attachment']);
							}
						}
					}
                                        */
                                        
	$rs = $adb->pquery("select first_name,last_name, email1 from vtiger_users where id=?", array($userId));
	...
	if($num_rows > 0){
            ...
            $from_email=$adb->query_result($rs, 0, 'email1');
                                        ...
                                        $mail_status= send_mail($module,$to_string,$userFullName,$from_email,$subject,$description,$ccs='',$bccs='',$attachments,$emailId,$logo='');

					 //$mail_status= MailSend($mailer);
				}
			}
